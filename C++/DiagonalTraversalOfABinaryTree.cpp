...
void traversal(Node * root , int diagonal , map<int , vector<int>> &map) {
    if(root == NULL) return;
    if(diagonal > maximumDiagonal) {
        maximumDiagonal = diagonal;
    }
    map[diagonal].push_back(root->data);
    traversal(root->left , diagonal + 1 , map);
    traversal(root->right , diagonal , map);
}
vector<int> diagonal(Node *root)
{
   ...
   vector<int> ans;
   for(int d = 0 ; d <= maximumDiagonal ; d++) {
       vector<int> list = map[d];
       for(int d : list) {
           ans.push_back(d);
       }
   }
